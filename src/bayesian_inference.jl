...
function bayesian_inference(prob::DEProblem,t,data;alg="integrate_ode_rk45",num_samples=1, num_warmup=1,kwargs...)
  ...
  data {
    real y0[2]; # 2 = length(prob.u0)
    int<lower=1> T;
    real y[T,2]; # 2 = length(prob.u0)
    real t0;
    real ts[T];
  }
  transformed data {
  ...
  model{
    real y_hat[T,2]; # 2 = length(prob.u0)
    sigma ~ inv_gamma(2, 3);
    theta[1] ~ normal(1.5, 1); #1=length(prob.f.params)
  ...
  stanmodel = Stan.Stanmodel(num_samples=num_samples, num_warmup=num_warmup, name="parameter_estimation_model", model=parameter_estimation_model);
  const parameter_estimation_data = Dict("y0"=>prob.u0, "T" => size(t)[1], "y" => data', "t0" => prob.tspan[1], "ts"=>t)
  return_code, chain_results = Stan.stan(stanmodel, [parameter_estimation_data]; CmdStanDir=CMDSTAN_HOME)
