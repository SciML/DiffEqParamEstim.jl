...
function multiple_shooting_objective(prob::DEProblem,alg,loss,regularization=nothing;priors=nothing,mpg_autodiff = false,discontinuity_weight=1.0,
  ...
  cost_function = function (p)
    ...
    if priors == nothing
      ...
      loss_val += loss(sol_new,priors,p)
    end
