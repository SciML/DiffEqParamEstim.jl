...
aggregate_data = convert(Array,VectorOfArray([generate_data(sol,t) for i in 1:100])) 
distributions = [fit_mle(Normal,aggregate_data[i,j,:]-aggregate_data[i,j-1,:]) for j in 2:200, i in 1:2 ]

obj = build_loss_objective(prob1,Tsit5(),LogLikeLoss(t,distributions,true),
                                     maxiters=10000,verbose=false)
