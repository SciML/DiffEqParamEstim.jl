...
result = Optim.optimize(cost_function_1, [1.0], Optim.BFGS(),abstol=1e-8,reltol=1e-8)
@test result.minimizer[1] ≈ 1.5 atol=3e-1

result_bfgs = Optim.optimize(cost_function_2, [1.2,2.7], Optim.BFGS(),abstol=1e-8,reltol=1e-8)
@test result_bfgs.minimizer ≈ [1.5;3.0] atol=3e-1

result_bfgs = Optim.optimize(cost_function_3, [1.3,0.8,2.8,1.2], Optim.BFGS(),abstol=1e-8,reltol=1e-8)
