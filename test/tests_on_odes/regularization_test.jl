...
optprob = Optimization.OptimizationProblem(cost_function_1, [1.0], lb=[0.5], ub=[2.5])
result = solve(optprob, Optim.BFGS())
@test result.u[1]≈1.5 atol=5e-1

optprob = Optimization.OptimizationProblem(cost_function_2, [1.2, 2.7], lb=[0.5, 1.0], ub=[2.5, 5.0])
result = solve(optprob, Optim.BFGS())
@test result.minimizer≈[1.5; 3.0] atol=5e-1

optprob = Optimization.OptimizationProblem(cost_function_3, [1.3, 0.8, 2.8, 1.2], lb=[0.5, 0.1, 1.0, 0.1], ub=[2.5, 2.0, 5.0, 2.0])
result = solve(optprob, Optim.BFGS())
