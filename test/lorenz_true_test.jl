...
# Generate Data
data_sol_short = solve(
    prob_short, Vern7(), saveat = t_short, reltol = 1.0e-12,
    abstol = 1.0e-12
)
data_short = convert(Array, data_sol_short)
data_sol = solve(prob, Vern7(), saveat = t, reltol = 1.0e-12, abstol = 1.0e-12)
...
obj_short = build_loss_objective(
    prob_short, Tsit5(), L2Loss(t_short, data_short),
    Optimization.AutoForwardDiff()
)
optprob = Optimization.OptimizationProblem(
    obj_short, [9.0, 20.0, 2.0], lb = xlow_bounds,
    ub = xhigh_bounds
)
...
obj_short = build_loss_objective(
    prob_short, Vern7(), L2Loss(t_short, data_short),
    Optimization.AutoForwardDiff(),
...
opt = Opt(:GN_ORIG_DIRECT_L, 3)
optprob = Optimization.OptimizationProblem(
    obj_short, GloIniPar, lb = first.(LooserBounds),
